...
def _section_nav(pdf_diff: PdfDiff, new_pages: list[Page]) -> list[dict]:
    ...
    skipped. TITLE anchors carry a `descriptor` — the title's full printed name,
    joined across the lines it wraps onto and de-hyphenated (`title_descriptor`,
    #49) — so the bare "TITLE I" can be labelled. The synthesized front-matter
    anchor (issue #33) has no printed line number to resolve against, so it maps
    to offset 0 — the front matter always sits at the very top of the text.
    """
    _, offsets = pdf_full_text_print(new_pages)
    sections: list[dict] = []
    anchors = pdf_diff.v2_anchors
    for i, a in enumerate(anchors):
        ...
        if a.kind == "title":
            entry["descriptor"] = title_descriptor(new_pages, anchors, i)
