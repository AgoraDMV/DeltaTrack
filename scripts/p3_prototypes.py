...
P1_FIXTURES = [
    ...
    (
        "smart-quotes",
        "The Secretary shall \"certify\" that 'all' requirements are met.",
        _curly_quotes("The Secretary shall \"certify\" that 'all' requirements are met."),
        "Possible drop (curly quotes)",
    ),
    (
        "hyphen-shift",
        "The multi-disciplinary review panel shall evaluate cross-functional capabilities.",
...
def main() -> None:
    ...
    print("\n--- P.1 Fixtures ---")
    print(
        f"{'Label':<30} {'Raw':>6} {'Norm':>6} {'Lev':>6} {'Jac':>6} {'CurrentClass':<15}"
        f" {'ReclassNorm':<12} {'ReclassLev':<12} {'ReclassJac'}"
    )
    print("-" * 120)
    ...
    for label, a, b, note in P1_FIXTURES:
        ...
        print(
            f"{label:<30} {rr:>6.4f} {nr:>6.4f} {lr:>6.4f} {jr:>6.4f} {rc:<15}"
            f" {'Y' if reclass_n else 'N':<12} {'Y' if reclass_l else 'N':<12} {'Y' if reclass_j else 'N'}{flag}"
        )
    ...
    print()
    fix_reclass_n = sum(
        1 for r in all_rows if r["source"] == "fixture" and r["would_reclassify_under_normalize"] == "Y"
    )
    ...
    if prebuilt.exists():
        ...
        print(
            f"{'ID':<16} {'Raw':>6} {'Norm':>6} {'Lev':>6} {'Jac':>6} {'CurrentClass':<15}"
            f" {'ReclassNorm':<12} {'ReclassLev':<12} {'ReclassJac'}"
        )
        print("-" * 120)
    else:
    ...
    for row in catalog_sample:
        ...
        flag_parts = []
        if reclass_n:
            flag_parts.append(f"norm:{rc}→{nc}")
        if reclass_l:
            flag_parts.append(f"lev:{rc}→{lc}")
        if reclass_j:
            flag_parts.append(f"jac:{rc}→{jc}")
        if flag_parts:
            note_parts.append("[" + "; ".join(flag_parts) + "]")

        print(
            f"{short_id[-16:]:<16} {rr:>6.4f} {nr:>6.4f} {lr:>6.4f} {jr:>6.4f} {rc:<15}"
            f" {'Y' if reclass_n else 'N':<12} {'Y' if reclass_l else 'N':<12} {'Y' if reclass_j else 'N'}"
        )
    ...
    print(
        f"  (b) lev_ratio:             {avg_ms(timing['lev']):.3f} ms"
        f"  (truncated at {MAX_LEV_SIZE_CATALOG} chars for catalog)"
    )
    ...
    for r in all_rows:
        if r["source"] != "fixture":
            continue
        if (
            r["would_reclassify_under_normalize"] == "Y"
            or r["would_reclassify_under_lev"] == "Y"
            or r["would_reclassify_under_jaccard"] == "Y"
        ):
            print(
                f"  [{r['fixture_or_diff_id']}] raw={r['raw_seq_ratio']}"
                f" norm={r['normalized_seq_ratio']} lev={r['lev_ratio']} jac={r['jaccard_ratio']}"
            )
