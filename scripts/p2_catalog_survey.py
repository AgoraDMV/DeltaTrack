...
def main() -> None:
    ...
    fieldnames = [
        ...
        "lev_ratio",
        "jaccard_ratio",
        "would_reclassify_with_casefold",
        "would_reclassify_with_whitespace",
        "raw_class",
        "casefold_class",
        "ws_class",
    ]
    ...
    if raw_false_rewrites > 0:
        ...
        print(
            f"  Would reclassify with casefold:         {reclassify_casefold}"
            f" ({100 * reclassify_casefold / raw_false_rewrites:.1f}%)"
        )
        print(
            f"  Would reclassify with whitespace:       {reclassify_whitespace}"
            f" ({100 * reclassify_whitespace / raw_false_rewrites:.1f}%)"
        )
        print()
    ...
    if examples_casefold:
        ...
        for i, ex in enumerate(examples_casefold, 1):
            ...
            print(
                f"    raw={ex['raw_ratio']}  casefold={ex['casefold_ratio']}"
                f"  lev={ex['lev_ratio']}  jaccard={ex['jaccard_ratio']}"
            )
