...
| `probe_node_identity.py` | Whether an ADR 0019 ordinal is still recoverable where matching runs: does `match_nodes` return the *same objects* its source `BillTree.nodes` holds, and is that recovery a bijection? Carries three negative controls, because a lookup that cannot miss proves nothing — a value-equal copy substituted for the real node, value collisions within one tree, and output position used as if it were the ordinal. |
| `probe_round2_migration.py` | What a pre-classification second retrieval round would have to reproduce. Measures the part `probe_splits.py` leaves open — how much of the move-candidate and selected-move population depends on an entry **created by classification** — and puts the legacy `(ri, ai)` sort key and the ADR 0019 ordinal key through one greedy loop to see whether they select the same correspondence. Refuses to report either figure unless its duplicated loop first reproduces production's selected set *and* order. |
| `probe_split_to_assignment.py` | Simulates moving the `SIMILARITY_THRESHOLD` revocation out of classification and into an explicit assignment pass, then asks whether anything moved. The predicate is **transcribed**, `diff_text` gate included, rather than tidied into a string comparison — the tidier form is carried alongside only so the two can be measured against each other, and their agreement on 27 documents is reported as exactly that rather than as a licence to substitute. Compares the simulated pre-reconcile change list against production's **element-wise, in order** (counts hide a reordering), asserts that classification preserves the correspondence shape it is handed, checks whether the legacy `(ri, ai)` order key is derivable from assignment state before classification runs, and re-asks the whole question at the canonical output. Four negative controls — revoke nothing, reversed emission order, a deliberately dropped side, and a predicate mutation against the committed baseline — must all redden, and the run exits non-zero if any does not. |
| `probe_canonical_sensitivity.py` | Proves the canonical byte-identity gate can actually redden on a *correspondence* change, by injecting one and watching the digests. Four passes: production, the duplicated loop under the production key (so a drifted copy cannot be mistaken for the fault), the duplicated loop under an ordinal key, and production restored. Fails loudly if the injected change moves no byte, since a gate that cannot see this class of change is unusable as the ADR 0020 acceptance criterion. |
