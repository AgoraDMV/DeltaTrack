...
| `probe_split_to_assignment.py` | Simulates moving the `SIMILARITY_THRESHOLD` split out of classification and into an explicit assignment pass, then asks whether anything moved. Compares the simulated pre-reconcile change list against production's **element-wise, in order** (counts would hide a reordering), checks whether the legacy `(ri, ai)` order key is derivable from assignment state before classification runs, and re-asks the question at the canonical output. Two negative controls — rejecting nothing, and emitting a rejected pairing addition-first — must both redden, and the run exits non-zero if either does not. |
